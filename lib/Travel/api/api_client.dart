...
class AirportApiClient {
  ...
  Future<Flights> fetchFlights(String inboundCity, String outboundCity, String outboundDate, String inboundDate) async {
    Flights flights;
    ...
    try {
      if (inboundDate != null && outboundDate != null) {
        // print("outboundCity API : $outboundCity");
        // final response = await http.get(
        //     "https://partners.api.skyscanner.net/apiservices/browsedates/v1.0/FR/eur/en-US/lond/pari/2021/2021?apikey=prtl6749387986743898559646983194",
        //     headers: _skyscannerHeaders);
        final response = await http.get(
            "https://partners.api.skyscanner.net/apiservices/browsedates/v1.0/TR/TRY/tur/$inboundCity/$outboundCity/$inboundDate/$outboundDate?apikey=prtl6749387986743898559646983194",
            headers: _skyscannerHeaders);
        if (response.statusCode == 200) {
          print(" RESPONSE BODY : ${response.body}");
          flights = Flights.fromJson(jsonDecode(response.body));
        } else {
          throw Exception("Failed to fetch flight ${response.statusCode}");
        }
      } else {
        final response = await http.get(
            "https://partners.api.skyscanner.net/apiservices/browsequotes/v1.0/TR/TRY/tur/$inboundCity/$outboundCity/anytime/anytime?apikey=prtl6749387986743898559646983194",
            headers: _skyscannerHeaders);
        if (response.statusCode == 200) {
          flights = Flights.fromJson(jsonDecode(response.body));
        } else {
